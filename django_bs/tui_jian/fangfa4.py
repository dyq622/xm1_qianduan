...
# 3. 将相似性矩阵和电影名称保存为 CSV 文件
def save_similarities_to_csv(movie_embeddings, movieID_encoder, movie_id_to_name, filename='movie_similarities1.csv'):
    # 计算相似性矩阵
...
# 示例：保存所有电影的相似性矩阵
def main(model, num_movies, movieID_encoder, movie_id_to_name):
    movie_embeddings = get_movie_embeddings(model, num_movies)
    save_similarities_to_csv(movie_embeddings, movieID_encoder, movie_id_to_name)

# 运行示例
main(model, num_movies, movieID_encoder, movie_id_to_name)