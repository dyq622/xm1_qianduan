...
class MovieRatingsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        movie_type = torch.tensor(row['电影类型编码'], dtype=torch.long)
        ...
        return user, movie, movie_type, num_reviews, normalized_rating, rating
...
ratings_df['电影类型编码'] = movie_type_encoder.fit_transform(ratings_df['电影类型'])
...
# 2. NCF模型定义
class NCF(nn.Module):
    def __init__(self, num_users, num_movies, num_movie_types, embedding_dim, hidden_units):
        super(NCF, self).__init__()
        # 嵌入层
        self.user_embedding = nn.Embedding(num_users, embedding_dim)
        self.movie_embedding = nn.Embedding(num_movies, embedding_dim)
        self.movie_type_embedding = nn.Embedding(num_movie_types, embedding_dim)
        
        # MLP层
        self.fc1 = nn.Linear(embedding_dim * 3 + 2, hidden_units[0])  # 电影嵌入 + 用户嵌入 + 电影类型嵌入 + 额外的特征
        self.fc2 = nn.Linear(hidden_units[0], hidden_units[1])
        self.fc3 = nn.Linear(hidden_units[1], 1)
        
        self.relu = nn.ReLU()
        
    def forward(self, user, movie, movie_type, num_reviews, normalized_rating):
        user_emb = self.user_embedding(user)
        movie_emb = self.movie_embedding(movie)
        movie_type_emb = self.movie_type_embedding(movie_type)
        
        x = torch.cat([user_emb, movie_emb, movie_type_emb, num_reviews.unsqueeze(1), normalized_rating.unsqueeze(1)], dim=1)
...
def train_model(model, dataloader, epochs=10, lr=0.001):
    ...
    for epoch in range(epochs):
        model.train()
        epoch_loss = 0
        for user, movie, movie_type, num_reviews, normalized_rating, rating in dataloader:
            optimizer.zero_grad()
            output = model(user, movie, movie_type, num_reviews, normalized_rating)
...
num_movie_types = len(movie_type_encoder.classes_)  # 电影类型的数量
model = NCF(num_users, num_movies, num_movie_types, embedding_dim, hidden_units)
